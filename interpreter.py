...
def syntax_analyzer():
	...
	if get_line(0)=="HAI": # check if program starts with HAI only
		...
		if i > -1:
			...
			output_console("error::expected KTHXBYE at: " + get_line(i)) # program has no KTHXBYE
	else: # program does not start with HAI
		output_console("error::expected HAI at: " + get_line(line_number))
...
def statement(is_code_block):
	...
	# OPERATIONS
		...
		if len(tokens[line_number]) < 2:
			...
			return False
		if eval_expr(copy.deepcopy(tokens[line_number])):
			line_number += 1
			return True
		else:
			return False
...
def operations(token_list, operation, typecast_to):
	...
	if operation!="NOT":
		...
		if type(rhs)==bool and rhs==False:
			return False
...
def typecast(token_list, dest):
	...
	if type(result)==bool and result==False: # typecast failed
...
def gimmeh(index):
	...
	if index + 1 < len(tokens[line_number]):
		...
		token = get_current_token(index)
		if token==None:
			output_console("error::expected variable at: " + get_line(line_number))
			return False
		...
		if token in symbols:
			...
			output_console("error::" + str(token) + " is an undeclared variable at: " + get_line(line_number))
			return False
...
def validate_blocks(start, end): # checks if blocks are valid and non-empty
	...
	for i in range(start, end):
		...
		if token=="OMG": # check if valid OMG
			...
			if len(line) > 7:
				# if expr, eval_expr will change the IT
				# since validate block only, do not change IT
				temp = symbols["IT"] 
...
def switch_case():
	...
	if has_oic:
		...
		has_gtfo = index_gtfo > -1 
		
		if has_gtfo:
			if get_line(index_gtfo)!="GTFO":
				output_console("error::expected EOL at: " + get_line(index_gtfo))
				return False
			gtfo.append(index_gtfo)

			index_gtfo += 1
			while index_gtfo < index_oic: # look for other gtfo
				index_gtfo = find_keyword("GTFO", index_gtfo, index_oic)
				if index_gtfo == -1: # error inside
					output_console("error at: " + get_line(line_number))
					return False
				elif index_gtfo == -2: # no more GTFO
					break
				if get_line(index_gtfo)!="GTFO": # not alone in its line
					output_console("error::expected EOL at: " + get_line(index_gtfo))
					return False
				gtfo.append(index_gtfo)
				index_gtfo += 1
		...
		while line_number < index_oic:
			if line_number in gtfo:
				break
			elif line_number in cases or get_line(line_number)=="OMGWTF":
				line_number += 1
			if not statement(True):
				return False
...
def loop():
	...
	variable = line[4]
	clause = line[5]
	temp_it = symbols["IT"] # in case loop has syntax errors
	expression = eval_expr(line[6:]) # and symbols[IT] changed
	temp_expr = symbols["IT"] # take note of the result of expr temporarily
	symbols["IT"] = temp_it # return the value of IT
	...
	if variable not in symbols.keys():
		...
		result = cast(symbols[variable], "NUMBR")
		if type(result)==bool and result==False:
	...
	if has_im_outta_yr:
		...
		# update variables
		symbols["IT"] = temp_expr
		symbols[variable] = result
		in_loop.append(True)
